...
const ProductDetails = () => {
    ...
    const productDetail = flippedProduct.map((product) => (
        <div key={product.id} className='px-8 lg:px-32 pb-20 flex flex-col w-full'>
            ...
            <Link to={`/${category}`} className=' text-start text-[#7d7d7d] w-full'><p>Go Back</p></Link>
            </div>
            <div className=' flex flex-col md:flex-row'>
                <span className=' w-full md:w-1/2 flex items-center justify-center bg-[#F1F1F1] rounded-md'>
                    <img className=' w-[173px] h-[225px] md:w-[296px] md:h-[385px] rounded-md' src={product.categoryImage.mobile} alt={product.name} />
                </span>
                ...
                <div className=' flex flex-col gap-3 justify-center items-start md:items-start pt-6 md:w-1/2 md:pl-8 md:gap-5'>
                    ...
                    <h1 className='lg:text-[40px] text-[22px] md:text-[28px] text-start font-semibold '>{product.name.toUpperCase()}</h1>
                    <p className=' text-xs text-start lg:text-left '>{product.description}</p>
                    <p className=' font-semibold'>$ {product.price}</p>
                    <div className=' w-full flex justify-between items-center'>
                        <div className=' w-1/3 bg-[#F1F1F1] py-3 flex justify-between items-center px-3 md:px-4 md:w-[130px]'>
                            <img onClick={decreaseQuantity} className=' w-[13px] h-[12px] cursor-pointer' src={remove} alt="remove" />
                            <input className='  text-black text-center w-full bg-inherit' type="number" placeholder={quantityOrdered} />
                            <img onClick={increaseQuantity} className=' w-[14px] h-[14] cursor-pointer' src={add} alt="add" />
                        </div>
                        <button onClick={()=> {addToCart(product.id)}} className=' bg-[#D87D4A] py-3 px-5 w-36 h-12 text-sm hover:bg-[#FBAF85] self-center md:self-start text-white'>ADD TO CART</button>
                    </div>
                </div>
            </div>
            ...
            <div className=' w-full flex flex-col gap-5 py-20 md:flex-row'>
                <div className=' flex flex-col gap-5'>
                    <span >
                        <img className=' rounded-md' src={product.gallery.first.mobile} alt="gallery image 1" />
                    </span>

                    <span>
                        <img className=' rounded-md' src={product.gallery.second.mobile} alt="gallery image 2" />
                    </span>
                </div>

                <div>
                    <img className=' rounded-md' src={product.gallery.third.mobile} alt="gallery image 2" />
                </div>
            </div>
                ...
                <div className=' flex flex-col md:flex-row gap-8 md:gap-4 w-full'>
                    {
                        product.others.map((other, index) => (
                            <div key={index} className='flex flex-col justify-center items-center w-full gap-8'>
                                <div className=' w-full flex justify-center items-center bg-[#F1F1F1] rounded-md'>
                                    <img className=' h-[100px] w-[95px] md:h-[320px] md:w-[220px]' src={other.image.mobile} alt={other.name} />
                                </div>
                                ...
                                <Link to='/Headphones/XXMARKII'><button className=' bg-[#D87D4A] py-3 px-5 w-36 h-10 text-sm mt-4 hover:bg-[#FBAF85] self-center md:self-start text-white'>SEE PRODUCT</button></Link>
                            </div>
                        ))
                    } 
                </div>
